...
pub async fn create_pr(
    ...
    // Thresholds are heuristic: titles should be >10 chars, body >20 chars.
    if pr_body.title.len() < 10 {
        ...
        pr_body.title = if improvements.is_empty() {
            ...
            // Enforce 72-character limit per requirements.
            truncate_to_char_limit(&title, 72)
        };
    }
    if pr_body.body.len() < 20 {
