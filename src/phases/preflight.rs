...
pub async fn run(repo_slug: &str, review_prs: bool, review_filter: &str, investigate_issues: &str) -> Result<PreflightOutput> {
    ...
    run_with_stale_threshold(repo_slug, stale_threshold, review_prs, review_filter, investigate_issues).await
}
...
/// * `review_prs` - Whether to review external PRs
/// * `review_filter` - Filter for which PRs to review
/// * `investigate_issues` - Label filter for issues to investigate
pub async fn run_with_stale_threshold(repo_slug: &str, fixing_stale_minutes: i64, review_prs: bool, review_filter: &str, investigate_issues: &str) -> Result<PreflightOutput> {
